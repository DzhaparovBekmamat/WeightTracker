...
import '../../utils/ui/bottomSheet/bottom_sheet_content.dart';
...
abstract class HomeViewModel extends State<HomeView>
  ...
  Future<void> _initialize() async {
    ...
    if (kDebugMode)
      print("First login: ${await widget.manager?.checkFirstLogin()}");
  ...
  Future<void> _getValues() async {
    ...
    for (var i = 0; i < dateList.length; i++) {
      setState(() => dataList.add(UserWeight(
          date: DateTime.parse(dateList[i]),
          weight: double.parse(weightList[i]))));
    }
  ...
  Future<dynamic> showCustomBottomSheet(BuildContext context) {
    ...
    return showModalBottomSheet(
        shape: RoundedRectangleBorder(
            borderRadius:
                BorderRadius.vertical(top: Radius.circular(radiusNormal))),
        context: context,
  ...
  bool isDatesEqual(DateTime lastDate, DateTime key) {
    return key.year == lastDate.year &&
        key.month == lastDate.month &&
        key.day == lastDate.day;
  }

  void applyWeight() {
    final String weightString =
        '${weightFormFieldController.text}.${weightSemiFormFieldController.text}';
