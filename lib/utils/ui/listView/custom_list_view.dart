...
class WeightListView extends StatefulWidget
    with ProjectStrings, ProjectPaddings, ProjectRadius {
  WeightListView({
    required Key key,
    required this.data,
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: widget.isLoading
          ...
          : widget.data.isEmpty
              ? Center(
                  child: Text(findString(
                      widget.languageIndex, LanguagesEnum.emptyData)))
              : ListView.builder(
                  itemCount: widget.data.length,
                  itemBuilder: (BuildContext context, int index) {
                    UserWeight currentData = widget.data[index];
                    bool isSelected =
                        selectedItems.contains(widget.data[index]);
        ...
        builder: (context) {
          return StatefulBuilder(
            builder: (context, setState) {
              return AlertDialog(
                title: Text(
                    findString(widget.languageIndex, LanguagesEnum.editTitle)),
                content: Row(
                  ...
                  children: [
                    Expanded(
                      ...
                      child: CustomFormField(
                        ...
                        onChanged: (text) {
                          isOkBtnActive =
                              weightFormFieldController.text.isNotEmpty;
                          setState(() {});
                        },
                ...
                actions: [
                  TextButton(
                    ...
                    child: Text(findString(
                        widget.languageIndex, LanguagesEnum.cancelUpper)),
                  ),
                  TextButton(
                    onPressed: isOkBtnActive
                        ...
                        : null,
                    child: Text(findString(
                        widget.languageIndex, LanguagesEnum.okUpper)),
                  ),
                ],
              );
            },
