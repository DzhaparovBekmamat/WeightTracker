...
class LineChartView extends StatefulWidget {
  const LineChartView(
      {super.key, required this.data, required this.languageIndex});
...
class _LineChartViewState extends State<LineChartView>
  ...
  void initVisibleData() {
    for (var element in widget.data) {
      if (element.date.year == widget.data.first.date.year) {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        OverflowBar(
            alignment: MainAxisAlignment.spaceAround, children: _getButtons()),
        Expanded(
          child: Card(
        ...
        case 0:
          if (widget.data.length >= 7) {
            for (int i = 0; i < 7; i++) {
              ...
              for (int j = 0; j < 7; j++) {
                var checkData =
                    widget.data.first.date.subtract(Duration(days: j));
        ...
        case 1:
          for (var element in widget.data) {
            if (widget.data.first.date.year == element.date.year) {
              if (widget.data.first.date.month == element.date.month) {
                visibleData.add(element);
              }
            }
          }
